...
export default function PhoneDemo() {
  ...
  // POST request for calling - FIXED: Changed phone_number to phone
  const makeCall = async () => {
    ...
    try {
      ...
      let phoneNumberToCall = cleanNumber;
      if (!cleanNumber.startsWith('+91') && cleanNumber.length === 10) {
        phoneNumberToCall = `+91${cleanNumber}`;
      } else if (cleanNumber.startsWith('91') && cleanNumber.length === 12) {
        phoneNumberToCall = `+${cleanNumber}`;
      } else if (cleanNumber.startsWith('0') && cleanNumber.length === 11) {
        phoneNumberToCall = `+91${cleanNumber.substring(1)}`;
      }

      // Log the number being called
      ...
      const response = await fetch('https://web-production-62a2b1.up.railway.app/api/call/start/', {
        ...
        body: JSON.stringify({
          phone: phoneNumberToCall,  // CHANGED: phone_number to phone (backend expects "phone")
          device_id: "iphone-demo-ui",
      ...
      if (response.ok) {
        ...
        console.error("Call failed with status:", response.status, "Response:", errorText);
  ...
  const endCall = async () => {
    ...
    try {
      ...
      await fetch('https://web-prodution-62a2b1.up.railway.app/api/call/end/', {
        ...
        body: JSON.stringify({
          phone: phoneNumberToCall,  // CHANGED: phone_number to phone
